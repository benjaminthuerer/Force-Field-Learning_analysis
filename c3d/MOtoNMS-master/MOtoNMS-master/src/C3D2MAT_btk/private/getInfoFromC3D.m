function [all, analogs, Markers, AnalogData, FPdata, Events, ForcePlatformInfo,Rates] = getInfoFromC3D(c3dFilePathAndName)
...
for i=1:numel(fields)
  if isstruct(s.(fields{i}))
      if strcmp(fields{i},'children')
          rs = recStruct(s.children);
          fields2 = fieldnames(rs);
            for j=1:numel(fields2)
                so.(fields2{j}) = rs.(fields{j});
            end
      elseif strcmp(fields{i},'info')
        rs = recStruct(s.info);
          fields2 = fieldnames(rs);
            for j=1:numel(fields2)
                so.(fields2{j}) = rs.(fields{j});
        ...
        so.(fields{i}) = recStruct(s.(fields{i}));
      end
  else
      so.(fields{i}) = s.(fields{i});
  end
end
end
